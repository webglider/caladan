...
for sz in 4 16 32 64 128; do 
    for qd in 1 2 4 8 16 32 64 128; do
        ...
        # Spawn IOkernel
        sudo NUMA_COUNT=1 ./iokerneld 2>&1 | tee ~/caladan-eval/$outlabel.iokernel &
        pids+=($!);
        ...
        # Start app
        sudo NUMA_COUNT=1 apps/bench/storage_client "$app.config" $target $(($sz*1024)) $qd $duration 1 100 > ~/caladan-eval/$outlabel.txt 2>&1 &
        pids+=($!);
        (sleep $record_sleep; sar -u 1 $record_duration -P $record_cores > ~/caladan-eval/$outlabel.util) &
        (sleep $record_sleep; pidstat -u -hl -p $(pgrep storage_client),$(pgrep -d, -P $(pgrep storage_client)) 1 $record_duration > ~/caladan-eval/$outlabel.apputil) &
        echo "Launched $app";
